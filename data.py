...
image = Image.open('D:\Data science\youtube.jpg')
st.image(image)
st.title (":red[Youtube Data Harvesting]") 


api_key='AIzaSyAHbvx8AkvFzi_8y-RQRWjdP3gQ8zbgK-k'
...
if choice=='Channel details':
    ...
    if submit:
        ...
        for i in v:
            vdids.append(i[0])
        # st.write(vdids)
        vd= video_data(youtube, vdids)
        st.dataframe(vd)
        cd=get_video_comments(youtube,vdids)
        # st.dataframe(cd)
        all_data_info=get_all_data(youtube, channel,vdids)
...
if choice=="Spicy Questions":
    ...
    st.write('This project is about is done by K.Rakul')
    URL="https:www.linkedin.com/in/rakul-k-9148241b5"
